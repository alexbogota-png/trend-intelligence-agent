...
const renderAnswer=answer=>{
  ...
  return `<div class="weekly-chat-answer"><section class="chat-insight-section chat-central"><h4>Idea central</h4><p>${chatEscape(answer?.idea_central||'')}</p></section>${chatList('Qué vemos',answer?.que_vemos)}${chatList('Qué significa',answer?.que_significa)}${chatList('Qué haría',answer?.que_haria)}<div class="chat-evidence-level">${chatEscape(answer?.nivel_evidencia||'')}</div></div>`;
};
...
const renderAssistant=item=>`${renderAnswer(item.content)}${renderVisual(item.visual)}`;
