...
const renderAnswer=answer=>{
  ...
  return `<div class="weekly-chat-answer"><section class="chat-insight-section chat-central"><h4>Respuesta directa</h4><p>${chatEscape(answer?.respuesta_directa||answer?.idea_central||'')}</p></section>${trendBlock(answer?.tendencia)}${chatList('Evidencia',answer?.evidencia||answer?.que_vemos)}${chatList('Interpretación',answer?.interpretacion||answer?.que_significa)}${connectionList(answer?.conexiones_portafolio)}${chatList('Acción sugerida',answer?.accion||answer?.que_haria)}<div class="chat-evidence-level">${chatEscape(answer?.nivel_evidencia||'')}</div></div>`;
};
